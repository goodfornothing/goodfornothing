  ...
  def show
    ...
    if @member.nil?
      ...
      @gig = (params[:gig_id].present?) ? Gig.find(params[:gig_id]) : nil
			...
			@history = (@member.comments + @member.contributions + @member.trills.published + @member.socials.past + @member.gigs.past).sort_by(&:audited_at).reverse.group_by{ |t| [t.audited_at.year,t.audited_at.month] }  
