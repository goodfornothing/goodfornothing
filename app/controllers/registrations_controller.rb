    ...
    def check_subscription(resource)
      if resource.activated?
        ...
        if resource.subscribed?
          mc.list_subscribe({:id => ENV['MC_LIST_ID'], :email_address => resource.email, :location => chapter.title, :merge_vars => { :GROUPINGS => [ { :name => 'Source', :groups => 'Platform' } ] }, :double_optin => false, :send_welcome => false })
