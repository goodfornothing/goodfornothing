    ...
    def check_subscription(resource)
      if resource.activated?
        ...
        if resource.subscribed?
          # if !chapter.empty?
            mc.list_subscribe({:id => ENV['MC_LIST_ID'], :email_address => resource.email, :merge_vars => { "MMERGE4" => chapter.title, :GROUPINGS => [ { :name => 'Source', :groups => 'Platform' } ] }, :double_optin => false, :send_welcome => false })
          # else
